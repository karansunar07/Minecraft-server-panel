...
export function SoftwarePage() {
  ...
  const { data: versions = [] } = useQuery({ queryKey: ["versions"], queryFn: () => serverApi.versions("paper") });

  return (
    <div className="mx-auto max-w-[1280px] space-y-6">
      <header>
        <h1 className="text-3xl font-extrabold">Software</h1>
        <p className="text-slate-400">Install server software through the backend provider.</p>
      </header>
        ...
        {software.map((item) => (
          <article key={item.id} className="rounded-[8px] bg-panel-card p-5 shadow-panel">
            ...
            <div className="mt-4 grid gap-3">
              <select className="rounded-[8px] border border-white/10 bg-black px-3 py-2">{versions.map((version) => <option key={version}>{version}</option>)}</select>
              ...
              <ActionButton variant="primary"><Download size={17} />Install</ActionButton>
            </div>
          </article>
        ))}
      </section>
